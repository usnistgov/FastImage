...
/// @file HTGSTileRequestData.h
/// @author Alexandre Bardakoff - Timothy Blattner
/// @date  7/28/17
/// @brief Data representing a tile request

#ifndef FASTIMAGE_HTGSTILEREQUESTDATA_H
#define FASTIMAGE_HTGSTILEREQUESTDATA_H

#include <ostream>
#include <htgs/api/IData.hpp>
#include <htgs/types/Types.hpp>

#include "HTGSViewRequestData.h"
...
/**
 * @class HTGSTileRequestData HTGSTileRequestData.h <FastImage/data/HTGSTileRequestData.h>
 * @brief Data representing a tile request.
...
class HTGSTileRequestData : public htgs::IData {
 public:
  /// \brief HTGSTileRequestData constructor
  /// \details Construct by a ViewLoader to a ATileLoader, the
  /// HTGSTileRequestData represent the Tile to load from the file (or cache if
  ...
  HTGSTileRequestData(uint32_t rowTileAsked,
                      uint32_t colTileAsked,
                      const m_data_t<View<UserType>> &viewData,
                      const std::shared_ptr<HTGSViewRequestData<UserType>>
                      &viewRequest)
      :
  ...
  const std::shared_ptr<HTGSViewRequestData<UserType>> &getViewRequest() const {
    return _viewRequest;
  }
  ...
  /// \brief Output stream operator for tge HTGSTileRequestData
  /// \param os Output stream
  /// \param data HTGSTileRequestData to print
  /// \return Output stream with the HTGSTileRequestData data
  friend std::ostream &operator<<(std::ostream &os,
                                  const HTGSTileRequestData &data) {
    os << "HTGSTileRequestData[" << *(data._viewRequest) << "]";
    return os;
  }
 ...
 private:
  ...
  std::shared_ptr<HTGSViewRequestData<UserType>>
      _viewRequest;       ///<View request generating the view data
};
}
#endif //FASTIMAGE_HTGSTILEREQUESTDATA_H
